...
### Added
- Added `437 :Nick/channel is temporarily unavailable` to the client-transparent swallow set, mirroring the existing `433` handler. `ratbox`-family ircds (charybdis, solanum) — most prominently EFnet's primary servers — implement a "nick delay" mechanism that places a recently-departed nick into a temporary lockout window after a netsplit or quit, typically for ~15 minutes, to prevent collision races during split-heal. During that window, the server rejects every `NICK <Primary>` attempt with `437` instead of `433`. Without swallowing, the IRC client receives a "Nickname is temporarily unavailable" message on each attempt for the duration of the delay, even though the module is doing exactly the right thing and will silently succeed once the lockout expires. The new handler intercepts these module-generated `437` responses before they reach the client, preserving the same client-quiet behavior the module already provides for `433`. The motivating real-world report came from an EFnet session where ZNC disconnected, the user reconnected as `ExamppleNick-`, the original ghost connection eventually quit via netsplit, and reclaim attempts then produced 15+ visible "Nickname is temporarily unavailable" messages in mIRC over the following minutes until the delay window expired and reclaim quietly succeeded.
- Added `437` to the `OnRaw` cheap pre-filter alongside `433`, preserving the no-allocation fast path for the ~99% of incoming lines (PRIVMSG/NOTICE/JOIN/PART/MODE/etc.) that the module does not handle.

### Changed
- Bumped version from `1.6.8` to `1.6.9`.
- The module now handles **9** command types in `OnRaw` instead of 8 (`005`, `303`, `730`, `731`, `734`, `421`, `433`, **`437`**, `NICK`, `QUIT`). All other lines continue to be rejected by the pre-filter before any vector allocation.
- Updated `TModInfo` description and `NETWORKMODULEDEFS` version string to `1.6.9` and to list `433/437/303/421 swallow` (was `433/303/421 swallow`).

### Documentation
- Updated the Feature summary `Client-transparent ... swallow` bullet to cover the `437` case, with a note about the EFnet/ratbox nick-delay scenario and the conservative gating that keeps user-typed `/nick` and channel-targeted `437`s passing through unchanged.
- Updated the behavior-notes paragraph in the implementation section to mention `437` alongside `433`/`303`/`421`.
- Updated the version reference in the intro line, the Default behavior section, and the Source summary block.

### Notes
- The swallow gate is intentionally identical to the `433` swallow: only fires when `NickAttemptPending` is set (the module's own `TryReclaim` armed it before sending `NICK <Primary>`) **and** `v[3]` (the rejected target in the `437` reply) `Equals(Primary, false)`. This shape gives correct-by-construction behavior in every edge case:
  - **Module-generated NICK during nick-delay window:** swallowed (the noise we are removing).
  - **User-typed `/nick ExampleNick` during nick-delay window:** passes through unchanged. `NickAttemptPending` is only set inside `TryReclaim` for module-initiated attempts; user-initiated `NICK` commands never set it.
  - **`437` for a channel target (e.g. juped channel):** passes through unchanged. `437` is also used for `JOIN #juped` rejections; in that case `v[3]` is the channel name, which cannot equal a user nick. The match-on-Primary check correctly distinguishes the two uses of `437`.
  - **`437` for an unrelated nick** (alias, alt, services-restricted target): passes through unchanged for the same reason.
- The `NickAttemptPending` flag is now cleared by either `433` or `437` in addition to its previous reset paths in `OnIRCConnected` / `OnIRCDisconnected`. This means a single NICK attempt that produces both a `433` and a `437` would only see the first one swallowed, but in practice servers send one or the other for a given attempt, never both. Same constraint already applies to the `433` path.
- This patch is **universal across networks**, not specific to EFnet. Undernet's `ircu` also returns `437` for some scenarios (juped/services-restricted nicks), and the gate is `NickAttemptPending && match-on-Primary`, so the swallow only fires on module-generated traffic regardless of which network is involved. The behavior on Undernet's normal ISON-only path is unchanged in every respect except that the small population of `437` responses for `Primary` (which would have been visible noise) is now silenced consistently with `433`.
- **Tested against ZNC 1.9.1.** The patched module compiles cleanly with `znc-buildmod` (no warnings or errors from this source file under `-Wall -Wextra -Wpedantic`), and was verified end-to-end against a real ZNC 1.9.1 instance and a fake `ratbox`-style IRC server. Five test scenarios all pass: (1) full 437 reject/accept cycle on EFnet-style MONITOR network with three rejected attempts followed by acceptance, all 437s correctly swallowed; (2) 433 regression test with the same shape, confirming the existing path is unchanged; (3) user-typed `/nick ExampleNick` during nick-delay window, 437 correctly passes through to client; (4) channel-target `437` for juped channel, 437 correctly passes through to client; (5) 437 swallow on Undernet-style ISON-only network, all 437s correctly swallowed and reclaim succeeds.
- No changes to the timer model, the `NickAttemptPending` flag semantics (still cleared on either 433 or 437 receipt), the `ISONQueryPending` counter, the `MonitorActive`/`MonitorUsable` match gating, or any default value. This release is narrowly scoped to the `437` swallow.
...
- Fixed `LoadPrimary()` fallback using `GetNetwork()->GetNick()` — the current IRC nick — when no stored `PrimaryNick` is available. On a hot reload via `updatemod` while connected under an alternate nick (e.g. `ExampleNick-`), this caused the module to poll for the wrong nick for the duration of that session. The fallback now uses `GetUser()->GetNick()`, which reflects the nick the user has configured in ZNC and is the correct authoritative source. `GetNV("PrimaryNick")` remains the first preference; the user-configured nick is only reached if no stored value exists.
