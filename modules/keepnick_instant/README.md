...
As of version **1.6.3**, the module supports two reclaim backends:
...
The module source currently identifies itself as version **1.6.3** and defaults to:
...
One deliberate side effect of the `433` swallow: client-side scripts that monitor for `433` to display a "nick in use" notification will not see failures caused by the module's own reclaim attempts. This is intentional — those notifications are noise when ZNC is handling reclaim automatically. Manual nick changes are unaffected.
...
- **Version:** `1.6.3`
- **Description:** Auto-backend, idle-aware, join-safe nick reclaim for ZNC (MONITOR when available, otherwise ISON; OnRaw pre-filter, consistent MONITOR subscription guards, hot-reload MONITOR detect, 433 swallow)
