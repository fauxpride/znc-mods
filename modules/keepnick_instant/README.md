...
As of version **1.6.9**, the module supports two reclaim backends:
...
- **Client-transparent 433, 437, 303, and 421 swallow**
  ...
  - `437 :Nick/channel is temporarily unavailable` responses generated by the module's own `NICK <Primary>` attempts are intercepted in the same way. This is most visible on `ratbox`/`charybdis`/`solanum` networks (e.g. EFnet) which place a recently-departed nick into a temporary lockout window after netsplits or quits to prevent collision races during split-heal. During that window, every `NICK <Primary>` attempt is rejected with `437` for up to ~15 minutes; without swallowing, the client would see a "Nickname is temporarily unavailable" message on every attempt until the delay expires and reclaim succeeds. `437` responses from manual nick changes, or with a channel target (e.g. juped channels), or for any nick other than `Primary` are always passed through unchanged.
  - `303` ISON poll responses generated by the module's own `ISON <Primary>` queries are intercepted before reaching the client, preventing them from appearing as noise in the IRC client status window. An integer counter (rather than a bool flag) tracks the number of outstanding module-sent ISONs, so multiple in-flight `303` responses are all correctly swallowed. Manual `/ison` commands typed by the user never increment the counter and are always passed through unchanged.
...
The module source currently identifies itself as version **1.6.9** and defaults to:
...
One deliberate side effect of the `433`, `437`, `303`, and `421` swallows: client-side scripts that monitor for `433` to display a "nick in use" notification will not see failures caused by the module's own reclaim attempts; `437` "Nick/channel is temporarily unavailable" responses from the module's own NICK attempts (common on EFnet and other `ratbox`-family networks during their post-netsplit nick-delay window) will not appear in the client; `303` ISON replies from the module's own polls will not appear in the client status window; and `421` responses to the module's own MONITOR probes will not appear as "Server does not recognize MONITOR command" noise. All are intentional — those responses are noise when ZNC is handling reclaim automatically. Manual nick changes, manual `/ison` commands, manual `MONITOR` commands, `437` for channel targets (juped channels), and any 433/437 for a nick other than `Primary` are always unaffected.
...
- **Version:** `1.6.9`
- **Description:** Auto-backend, idle-aware, join-safe nick reclaim for ZNC (MONITOR when available, otherwise ISON; OnRaw pre-filter, consistent MONITOR subscription guards, hot-reload MONITOR detect, 433/437/303/421 swallow, timer cleanup, ISON counter, nick validation, match-gated MONITOR state)
