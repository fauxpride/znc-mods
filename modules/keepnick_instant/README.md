...
As of version **1.6.6**, the module supports two reclaim backends:
...
- **Client-transparent 433, 303, and 421 swallow**
...
The module source currently identifies itself as version **1.6.6** and defaults to:
...
One deliberate side effect of the `433`, `303`, and `421` swallows: client-side scripts that monitor for `433` to display a "nick in use" notification will not see failures caused by the module's own reclaim attempts; `303` ISON replies from the module's own polls will not appear in the client status window; and `421` responses to the module's own MONITOR probes will not appear as "Server does not recognize MONITOR command" noise. All are intentional — those responses are noise when ZNC is handling reclaim automatically. Manual nick changes, manual `/ison` commands, and manual `MONITOR` commands are always unaffected.
...
- **Version:** `1.6.6`
- **Description:** Auto-backend, idle-aware, join-safe nick reclaim for ZNC (MONITOR when available, otherwise ISON; OnRaw pre-filter, consistent MONITOR subscription guards, hot-reload MONITOR detect, 433/303/421 swallow, timer cleanup)
